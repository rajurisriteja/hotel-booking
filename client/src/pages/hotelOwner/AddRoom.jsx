...
const AddRoom=()=>{
    ...
    return(
        <form>
           ...
           <div className="grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap cursor-pointer">
                {Object.keys(images).map((key)=>(
                    <label htmlFor={`roomImage${key}`} key={key}>
                        <img src={images[key] ? URL.createObjectURL(images[key]):assets.uploadArea} alt="" className="cursor-pointer max-h-14 opacity-80"/>
